...
- **Face Identity Consistency (HIGHEST PRIORITY)**: The face of each character MUST be identical to their reference portrait. This means: same nose shape, same eye shape and spacing, same jawline, same skin tone, same hair texture and style, same glasses (if any), same facial hair (if any). The face must look like the SAME PERSON, not just a similar-looking person. When generating the text prompt, always include an explicit instruction like: "The character's face MUST be identical to the reference portrait - same exact facial structure, features, and distinguishing marks."
...
[Guidelines]
- **FACE IDENTITY IS THE #1 PRIORITY**: Always include the character's front-view portrait as a reference when the character's face is visible. In your text_prompt, explicitly state: "This character's face MUST be IDENTICAL to Image N - same exact nose, eyes, jawline, skin tone, hair, and all distinguishing features like glasses or facial hair." Use hyper-detailed physical descriptions (e.g., "young South Asian man with curly dark hair, round glasses, light mustache") in the prompt.
...
[Guidelines]
- **FACE IDENTITY IS THE #1 PRIORITY**: Always include the character's front-view portrait as a reference when the character's face is visible. In your text_prompt, explicitly state: "This character's face MUST be IDENTICAL to Image N - same exact nose, eyes, jawline, skin tone, hair, and all distinguishing features like glasses or facial hair." Use hyper-detailed physical descriptions in the prompt to reinforce identity.
- Ensure that the language of all output values (not include keys) matches that used in the frame description.
- The reference image descriptions may depict the same character from different angles, in different outfits, or in different scenes. Identify the description closest to the version described by the user
- Prioritize image descriptions with similar compositions, i.e., shots taken by the same camera.
- The images from prior frames are arranged in chronological order. Give higher priority to more recent images (those closer to the end of the sequence).
...
class ReferenceImageSelector:
    ...
    async def select_reference_images_and_generate_prompt(
        ...
        try:
            ...
            # Clamp indices to valid range to handle LLM hallucinating out-of-range indices
            max_idx = len(filtered_image_path_and_text_pairs) - 1
            valid_indices = [i for i in response.ref_image_indices if 0 <= i <= max_idx]
            if not valid_indices and filtered_image_path_and_text_pairs:
                valid_indices = list(range(len(filtered_image_path_and_text_pairs)))
